...
## LaTeX-Ausgabe für Terme
```{r output4latex}
term2latex <- function(expr, prefix = "") {
  # Beispiel: expr = "2*x"
  # Gib zurück: $ 2\cdot x $
  
  # Wenn du die Ableitung berechnen willst:
  str4expr <- yac_str(expr)

  # Ersetze * durch nichts (für schönere Darstellung)
  latex_expr <- gsub("\\*", "\\cdot ", str4expr)
  
  # Gib LaTeX-Formel zurück
  return(latex_expr)
}
```
...
```{r , echo=TRUE}
extract_math <- function(text) {
  # Verwende regulaere Ausdruecke in Perl-Syntax
  regx4perl <- TRUE
  # Prüfe, ob text ein character-Vektor ist
  if (!is.character(text)) {
    stop("Der Input muss eine Zeichenketten sein.")
  }
  
  # Pruefe, ob überhaupt ein $-Zeichen vorhanden ist
  if (!any(grepl("\\$", text, perl = regx4perl))) {
    # Kein $ vorhanden → gib Originaltext zurück
    return(text)
  }
  
  # Regex: $...$ mit beliebigen Zeichen dazwischen (außer $)
  pattern <- "\\$[^$]*\\$"
  
  # Finde alle Vorkommen mit gregexpr
  matches <- regmatches(text, gregexpr(pattern, text, perl = regx4perl))
  
  # Entferne leere Einträge (falls vorhanden)
  formeln <- unlist(matches)
  formeln <- formeln[formeln != ""]  # Filtere leere Strings
  
  # Rückgabe: Vektor der Formeln (oder Originaltext, wenn kein $)
  return(formeln)
}

yacas2latex <- function(pYacasCmd) {
   ### YACAS-Befehl zu Latex-Erzeugung "TexForm( 5 * x^6 )"
   cmd4latex <- paste("TexForm(",pYacasCmd,")")
   ### Absenden des YACAS-Befehl zur Latex-Konvertierung an YACAS
   ### Ergebnis wird als String ausgegeben 
   ergebnis <- yac_str(cmd4latex)
   ergebnis <- extract_math(ergebnis)
